...
export default function ValidAccount({ navigation }) {
  ...
  function handleSubmit() {
    if ( code.length !== 4 ) {
      Alert.alert('Erro na validação', 'O código deve conter 4 dígitos.');
      return;
    }

    api.put(`users/email/${email}`, {code: code}).then(() => {
      dispatch(signInRequest(email, password, navigation));

    }).catch(err => {
      if ( err.response.status === 406 ) {
        Alert.alert('Erro na validação', 'Código não confere, tente novamente.');
      } else {
        Alert.alert('Erro na validação', 'Serviço indisponível, tente mais tarde.');
      }
    });
  }

  function newCode() {
    api.post(`users/email/${email}`).then(() => {
    ...
    <BackgroundInitial>
      ...
      <Container>
        ...
        <View style={styles.container}>
          <Text style={styles.title}>Verifique caixa de spam ou lixo eletrônico.</Text>
          <Text style={styles.title}>Código enviado para {email}</Text>
        </View>
        ...
        <SignLink onPress={newCode}>
          <SignLinkText>Enviar novo código</SignLinkText>
        </SignLink>
      </Container>
    </BackgroundInitial>
  );
